...
# Use this file to fine-tune your MonkDB cluster.

################################ Quick Settings ##############################

# Recommended memory settings:
# - set the environment variable MONKDB_HEAP_SIZE to 25% of your memory
#   (e.g. 16G, but not more than ~30G to benefit from CompressedOops).
#   Depending on the OS update '/etc/default/monkdb' or '/etc/sysconfig/monkdb'
#
#   for more details.
#
# - disable swapping
...
# Storage: MonkDB can utilize multiple volumes in parallel, make sure to set
# the owner to 'monkdb:monkdb'
...
# Cluster discovery: Specify the hosts which will form the MonkDB cluster
#discovery.seed_hosts:
#    - host1
#    - host2

# Bootstrap the cluster using an initial set of master-eligible nodes. All
# master-eligible nodes must be set here for clusters upgraded from MonkDB <
...
# To allow authenticated access to MonkDB from specific hosts, you must
...
# Relative paths are relative to MONKDB_HOME. Absolute paths override this
# behavior.
...
# MonkDB performs poorly when JVM starts swapping: you should ensure that
# it _never_ swaps.
#
# Set this property to true to lock the memory:
#bootstrap.memory_lock: true

# Make sure that the machine has enough memory to allocate for MonkDB,
# leaving enough memory for the operating system itself.
# You can allocate memory for MonkDB as follows:
#  - Set MONKDB_MIN_MEM and MONKDB_MAX_MEM environment variables
#    (We recommend to set MIN and MAX to the same value).
#  - Set MONKDB_HEAP_SIZE environment varaible. This sets MIN and MAX to the
#    same value for you.
#
# You should also make sure that the MonkDB process is allowed to lock
# the memory, eg. by using `ulimit -l unlimited`.


############################## Network And HTTP ###############################

# MonkDB, by default, binds itself to the loopback addresses on the system,
...
# Service discovery allows MonkDB to look up hosts for unicast discovery via
# SRV DNS records.
...
# EC2 discovery allows MonkDB to look up hosts for unicast discovery via the
# AWS EC2 API.
...
# Azure discovery allows MonkDB to look up hosts for unicast discovery via
# the Azure API.
...
# Defines the threshold on which MonkDB enforces a read-only block on every
...
# server v10.5.
#
# This allows you to connect to MonkDB using one of the many Postgres tools
# and libraries
#psql.enabled: true
#psql.port: 5432
```
